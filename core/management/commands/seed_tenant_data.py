...
# IMPORTA aquí tus modelos por-tenant
# from security.models import Dashboard, ...
# from user.models import ... (usa siempre get_user_model() cuando puedas)

def run_seed_logic():
    """
    Mueve aquí el contenido de tu 'core/tests.py' que crea
    dashboards, grupos, permisos, usuarios iniciales, etc.
    Este código se ejecutará ya dentro del tenant_context correcto.
    """
    User = get_user_model()
    ...
    usuario_cliente.is_staff = False
    usuario_cliente.set_password('cliente1234')
    usuario_cliente.save()
    group = Group.objects.get(pk=2)
    usuario_cliente.groups.add(group)


    cliente = Client()
    cliente.user = usuario_cliente
    cliente.birthdate = '2000-01-01'
    cliente.address = 'Ciudad Generica'
    cliente.mobile = ''.join(random.choices(numbers, k=7))
    cliente.save()
    print(f'Bienvenido {usuario_cliente.names}')


class Command(BaseCommand):
    help = "Seed inicial para módulos/grupos/usuarios DENTRO de schemas de tenants."

    def add_arguments(self, parser):
        parser.add_argument("--schema", help="Schema del tenant a sembrar (opcional)")
        parser.add_argument("--all-tenants", action="store_true", help="Correr en todos los tenants")
    ...
    def handle(self, *args, **opts):
        ...
        if opts["all_tenants"]:
            ...
            self.stdout.write(self.style.NOTICE(f"Sembrando en {tenants.count()} tenants..."))
            for t in tenants:
                self.stdout.write(f"→ {t.schema_name}")
                with tenant_context(t):
                    run_seed_logic()
            self.stdout.write(self.style.SUCCESS("OK (todos los tenants)"))
            return

        schema = opts.get("schema")
        if not schema:
            self.stderr.write("Debe pasar --schema=<schema> o --all-tenants")
            return

        try:
            t = TenantModel.objects.get(schema_name=schema)
        except TenantModel.DoesNotExist:
            self.stderr.write(f"No existe el tenant/schema: {schema}")
            return

        self.stdout.write(f"Sembrando en tenant '{schema}'...")
        with tenant_context(t):
            run_seed_logic()
