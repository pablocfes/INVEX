        ...
        tblUsers = $('#data').DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        row.groups.forEach(function (value, index, array) {
                            content += '<span class="badge badge-success">' + value.name + '</span> ';
                        });
                        return content;
                    }
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        var content = '<div class="dropdown"><div class="btn-group" role="group"><button id="btnGroupDrop1" type="button" class="btn btn-secondary btn-xs dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-align-justify"></i> Opciones</button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop1">';
                        ...
                        content += '<a class="dropdown-item" rel="update_password"><i class="fas fa-lock"></i> Cambiar password</a>';
                        content += '</div></div></div>';
                        return content;
                    }
                },
...
$(function () {
    ...
    $('body table')
        .on("hide.bs.dropdown", ".dropdown", function (event) {
            $(this).closest('table').parent().css('height', '');
        })
        .on("show.bs.dropdown", ".dropdown", function (event) {
            $(this).closest('table').parent().css('height', '65vh');
        });
});
